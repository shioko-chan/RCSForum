Page({
  ...
  formatTime(h, m, s) {
    ...
    return `${hours}:${String(m).padStart(2, "0")}:${String(s).padStart(
      2,
      "0"
    )}`;
  },
  ...
  showAccumulate() {
    if (
      this.data.ranks_acc_timestamp + 5 * 60 * 1000 > Date.now() &&
      this.data.ranks.length > 0
    ) {
  ...
  showSidebar() {
    if (
      this.data.ranks_timestamp + 5 * 60 * 1000 > Date.now() &&
      this.data.ranks.length > 0
    ) {
